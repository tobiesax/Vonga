-- Vonga-only schema for a dedicated Supabase project.
-- Same multi-tenant tables as the original bakery schema (001_initial_schema.sql)
-- plus the Vonga-specific columns from 002_vonga_columns.sql, combined into one
-- migration with the Crunch & Crumbs seed data removed. Nothing else changed —
-- app code (lib/repository.ts) reads/writes these tables exactly as before.
...
insert into public.merchants (slug, name) values ('vonga', 'Vonga') on conflict do nothing;
